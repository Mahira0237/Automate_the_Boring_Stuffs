...
logoWidth, logoHeight=LogoIm.size
os.makedirs('withLogo2', exist_ok=True)
#Loop over all files in the working directory.
for filename in os.listdir('.'):
    if not (filename.endswith('.png') or filename.endswith('.jpg') or 
            filename.endswith('.PNG') or filename.endswith('.JPG') or 
            filename.endswith('.gif') or filename.endswith('.GIF') or 
            filename.endswith('.bmp') or filename.endswith('.BPM')) or filename == LOGO_FILENAME:
    ...
    if width>=(2*logoWidth) and height>=(2*logoHeight):
        ...
        # Save changes.
        im.save(os.path.join('withLogo2', filename))